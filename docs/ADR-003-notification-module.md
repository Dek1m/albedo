...
| Поле | Значение |
|------|----------|
| **Статус** | accepted — вердикты Мастера от 02.09.2026 (schema review Норы внесено в ревизию 3; security review Литы — фронт-требования внесены в ревизию 4; ревизия 5 — bell-dropdown контракт Мастера; передана Соне) |
| **Дата** | 2026-09-02 (accepted 02.09.2026; ревизия 5 — bell-dropdown, вердикты Мастера, вечер) |
...
| **Ревизия 4** | security review Литы (02.09.2026), фронт-требования (§9.3): payload только текстовыми узлами (XSS), bidi-нейтрализация имён, display_name без нормализации, disabled-иконка без скрытия строки, Everyone-пометка + confirm, не рендерить id/level в тостах, resolve_entities UI-статусы. PENDING MASTER (домен fs, ADR-002): находка 1 (unix_name) и вердикт по находке 11 (Trash-семантика) — ожидают решения Мастера *(пометка историческая: находка 11 закрыта ревизией 5 ADR-002)* |
| **Ревизия 5** | bell-dropdown контракт Мастера (02.09.2026, вечер, §10.5): NotificationBell — **СПРАВА от UserChip** (в `.albedo-header-actions` ПОСЛЕ, не перед); клик по колокольчику — **НЕ mark_all_read**, а dropdown-панель списка уведомлений: элементы {текст из payload (правила §9.3), created_at, unread-индикатор}, клик по элементу → `mark_read(элемента)`, кнопка «Прочитать все» → `mark_all_read`, пагинация курсором из `notification.list`, пустое состояние «Нет уведомлений»; бейдж unread над иконкой (правый верхний угол), >99 → «99+», 0 → скрыт. Шаг 7 плана Соны (§16) обновлён |
...
Новый модуль `mia/modules/notification`: таблица `notification.notification` в системной БД belle (персистентные уведомления — «не теряются после офлайна»), фасад `state.notification.send(recipients, type, payload, bucket)` для других модулей (первый вызыватель — fs после `share_add`), self-scope RPC `notification.list / list_unread_count / mark_read / mark_all_read` на воркере, доставка в albedo — polling `list_unread_count` (30 c) поверх существующего toast-стека (внизу справа) **+ колокольчик справа от UserChip: бейдж unread над иконкой и dropdown-панель списка уведомлений (клик по элементу = mark_read, кнопка «Прочитать все» = mark_all_read), звук на новый share_grant-тост, клик по тосту = пауза исчезновения**; Everyone-гранты — асинхронная батчевая раздача воркером `notification.distribute`; TTL 30 дней, cleanup — периодическая воркерная задача; WebSocket и внешние каналы (email/push) — волна N+1.
...
### 9.3. Security review Литы — фронт-требования (ревизия 4)

Требования к рендерингу payload и ACL-данных в albedo. Все данные, приходящие в `payload` (`actor_name`, `node_name`) и в `fs.share_list` / `fs.resolve_entities` (`name`, `display_name`), — **враждебный ввод**: имена файлов и юзеров заполняются людьми, кириллица и unicode разрешены (вердикт №8 ADR-002). Фронт обязан:

| # | Требование | Детали |
|---|------------|--------|
| 1 | **XSS: только текстовые узлы** | `payload.actor_name` и `payload.node_name` рендерятся ТОЛЬКО как текстовые узлы (React text children). **`dangerouslySetInnerHTML` в тостах и дереве — ЗАПРЕЩЁН.** Никаких исключений «для жирного выделения имён» — выделение делается разметкой вокруг текстового узла, не внутри |
| 2 | **Bidi-нейтрализация при отображении** | Символы U+202A–U+202E (LTR/RTL-override) и U+2066–U+2069 (isolate) в именах заменяются видимым маркером **⧎** — подмена невидимого управления видимым артефактом ломает spoofing-атаки (имя «file.txt» с RTL-override, визуально читаемое наоборот). Контейнеры имён (тосты, ACL-таблица, дерево) — `dir="auto"` + `unicode-bidi: plaintext`, чтобы вперемешку с кириллицей/арабицей порядок задавал первый сильный символ строки, а не контекст страницы |
| 3 | **`display_name` — как есть, без нормализации** | Никаких trim/NFKC/casefold/lower — нормализация ломает аутентичные имена и создаёт ложные коллизии. Длинные имена — CSS `text-overflow: ellipsis` + tooltip с полным именем (полная строка — тоже текстовый узел, требование 1 действует) |
| 4 | **Disabled-получатель: строку не скрывать** | `active: false` в `share_list` → иконка отключённого пользователя + tooltip (например, «Пользователь отключён»), строка ACL-таблицы остаётся видимой. Скрытие строки прячет от владельца факт существования гранта — он должен видеть и уметь отозвать |
| 5 | **Everyone — явная пометка + confirm-диалог** | В ACL-таблице Everyone-грант помечается явно (вердикт §20.8 ADR-002). Перед `share_add` с Everyone — confirm-диалог с **канонической формулировкой из ADR-002 §5.5**: «Папка станет доступна всем пользователям. Они увидят ваше имя и имя папки.» |
| 6 | **Технические поля в тостах не рендерить** | `node_uuid`, `actor_id`, `level` в тостах не отображаются (вердикт 6 §9.2: перехода по узлу нет, поля — данные payload). Неизвестный `type` → нейтральный текст fallback'а §9.2 («Уведомление: {type}»), без попытки отформатировать незнакомый payload |
| 7 | **resolve_entities UI-статусы** | `unresolved` — красная строка (§10.3), `ambiguous` — список `candidates` с выбором «один / несколько / все» (вердикт №7 ADR-002). `email` в candidates отображается как текст; **не превращать в `mailto:` без явного действия пользователя** — резолв по email не значит согласие на клик-переход в почтовый клиент |

Требования 1–2 — блокирующие для шагов 6–8 плана §16 (фронт): ревью Литы перед слиянием фронт-кода.
...
### 10.5. NotificationBell: dropdown-панель, бейдж, звук и клик-пауза тоста (вердикты 2 и 6, 02.09.2026; dropdown-контракт — ревизия 5)

**Колокольчик: позиция и бейдж.** Новый компонент `src/features/notifications/NotificationBell.tsx`, размещается в `AppShell.tsx` внутри `.albedo-header-actions` **ПОСЛЕ** `<UserChip />` (AppShell.tsx:107-108) — **справа от UserChip**, вердикт Мастера (ревизия 5; прежний вариант «перед UserChip, слева» отменён). Иконка `bi-bell` / `bi-bell-fill` (bootstrap-icons уже в проекте). Бейдж unread — **над колокольчиком**: абсолютное позиционирование поверх иконки, правый верхний угол (классический badge-оверлей), значение = `count` из того же poller'а (`list_unread_count`, 30 c — отдельный запрос не нужен):

- `count > 0` → видимый бейдж с числом;
- `count > 99` → текст «99+» (порог отображения);
- `count = 0` → бейдж скрыт.

**Клик по колокольчику — НЕ `mark_all_read`, а dropdown-панель списка уведомлений** (вердикт Мастера, ревизия 5; прежний контракт «клик обнуляет всё» снят — обнуление стало осознанным действием внутри панели):

- **Открытие панели** — dropdown под иконкой; закрытие — клик вне панели / повторный клик по колокольчику / Escape.
- **Элемент списка** = `{ текст из payload (форматирование §9.2: только текстовые узлы, правила §9.3 — bidi-нейтрализация ⧎, ellipsis + tooltip, ничего не рендерить из `node_uuid`/`level`/`actor_id`), время `created_at`, индикатор unread (визуальная метка непрочитанного, напр. точка/подсветка строки) }`.
- **Клик по элементу** → `notification.mark_read(id)` → элемент теряет индикатор unread; бейдж уменьшается локально без ожидания тика. Никакой навигации по `node_uuid` — переход в папку отклонён Мастером навсегда (вердикт 6, §9.2).
- **Кнопка/действие «Прочитать все»** → `notification.mark_all_read()` → все индикаторы снимаются, бейдж обнуляется локально.
- **Пагинация/подгрузка** — курсором из `notification.list` (`next_cursor` из §10.4): скролл к низу панели → `list(limit, before=cursor)`; «новые сверху» по `unread_first`-порядку RPC.
- **Пустое состояние** — «Нет уведомлений» (панель открывается и при `count = 0`).
- Обновление списка при новых уведомлениях — poller'ом: новые items добавляются в шапку панели, пока она открыта; бейдж живёт по `count` независимо от состояния панели.
...
| 7. Фронт: NotificationBell + dropdown | `src/features/notifications/NotificationBell.tsx` в `AppShell.tsx` **после** `<UserChip />` (справа от него, §10.5): иконка bi-bell + бейдж над иконкой (правый верхний угол, поверх иконки; >0 виден, >99 → «99+», 0 → скрыт) из count poller'а. Клик по колокольчику → **dropdown-панель списка** (не mark_all_read!): элементы {текст из payload по правилам §9.2/§9.3, created_at, unread-индикатор}; клик по элементу → `mark_read(id)`; кнопка «Прочитать все» → `mark_all_read`; подгрузка курсором `next_cursor` при скролле; пустое состояние «Нет уведомлений»; закрытие — клик вне / Escape | Бейдж отражает unread в ≤30 c; панель открывается, элемент гасит свой unread, «Прочитать все» обнуляет бейдж; курсорная подгрузка без дублей |
...
11. **Бейдж + dropdown (ревизия 5)**: 150 непрочитанных → бейдж «99+»; клик по колокольчику → панель открылась, `mark_all_read` НЕ вызван (счётчик не изменился до действия пользователя); клик по одному элементу → `mark_read`, его индикатор погас, бейдж −1; «Прочитать все» → бейдж скрыт; скролл панели к низу → подгрузка курсором без дублей; пустой список → «Нет уведомлений»; клик вне панели / Escape → закрыта.
...
| 7 | **(ревизия 5)** Поведение колокольчика | **Dropdown вместо обнуления**: клик по NotificationBell открывает панель-список (элемент = текст+время+unread; клик по элементу = `mark_read`; кнопка «Прочитать все» = `mark_all_read`; курсорная пагинация; пустое состояние «Нет уведомлений»); бейдж над иконкой (правый верхний угол, >99 → «99+», 0 → скрыт); позиция — справа от UserChip | §10.5, §16 шаг 7 |

*(Ревизия 5 ADR-002: PENDING-вопрос находки 11 (Trash-семантика, домен fs) закрыт — ADR-003 не затрагивает; находка 1 (unix_name) остаётся открытой в ADR-002.)*
...
| 8 | **Бейдж + dropdown (ревизия 5)**: колокольчик справа от UserChip; бейдж над иконкой отражает unread (`count` из `list_unread_count`): >0 виден, >99 → «99+», 0 → скрыт; клик по колокольчику открывает dropdown-панель (не обнуляет); `mark_read` по элементу гасит его unread; кнопка «Прочитать все» → `mark_all_read` обнуляет бейдж; пустое состояние «Нет уведомлений» |
